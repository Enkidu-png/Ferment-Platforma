...
- [x] **Phase 1: Foundation** - Supabase project, PostgreSQL schema, RLS policies, JWT custom claims hook, and client factories (completed 2026-03-06)
- [x] **Phase 2: Auth Migration** - Supabase Auth replaces Payload auth; middleware and tRPC context updated (completed 2026-03-06)
- [x] **Phase 3: Data Migration** - Seed script with admin + 3 artists + categories + 20+ products; Playwright smoke tests (completed 2026-03-06)
...
- [x] 02-02-PLAN.md — Middleware composition (Supabase session refresh + subdomain routing) + subdomain cookie config
- [x] 02-03-PLAN.md — Sign-in/sign-up views rewire + /auth/confirm route + /pending page
...
**Plans**: 2 plans
Plans:
- [x] 03-01-PLAN.md — Seed script (admin + artists + categories + products) + .env.local credentials
- [x] 03-02-PLAN.md — Playwright install + smoke tests (auth routes + storefront subdomain)
...
**Plans**: 5 plans
Plans:
- [x] 04-01-PLAN.md — categories + tags + tenants routers + subcategory-menu.tsx (Payload types removed)
- [x] 04-02-PLAN.md — products router (two-step filters: tenant slug, category, tags)
- [x] 04-03-PLAN.md — reviews + library routers (user_id/product_id field names)
- [x] 04-04-PLAN.md — checkout router (user_tenants join) + Stripe webhook (supabaseAdmin)
- [x] 04-05-PLAN.md — remove ctx.db from tRPC init + delete Payload files + compile + Playwright green
...
**Plans**: 3 plans
Plans:
- [x] 05-01-PLAN.md — Storage bucket + RLS SQL migration + test scaffold (storage.spec.ts + verify-blob-urls.ts)
- [x] 05-02-PLAN.md — next.config.ts remotePatterns + mediaRouter.createRow tRPC mutation
- [x] 05-03-PLAN.md — Seed extension with picsum image uploads linked to products via image_id
...
**Plans**: 5 plans
Plans:
- [x] 06-01-PLAN.md — Test scaffold + adminProcedure + admin layout/routing
- [x] 06-02-PLAN.md — Merchants procedures + Tinder review UI
- [x] 06-03-PLAN.md — Products procedures + archive/restore UI
- [x] 06-04-PLAN.md — Categories + Tags procedures + CRUD UI
- [x] 06-05-PLAN.md — Orders procedure + router wiring + human verification
...
Plans:
- [x] 07-01-PLAN.md — Delete dead Payload files + remove packages + fix scripts + local build green
- [x] 07-02-PLAN.md — Vercel env vars + production deployment + Playwright smoke tests
- [x] 07-03-PLAN.md — Password reset script + dry-run verification + live artist notification
- [x] 07-04-PLAN.md — [GAP] Fix cookie domain guard + update storefront smoke tests + admin panel human verify
- [x] 07-05-PLAN.md — [GAP] artist1 password reset omitted (test account; set via Supabase dashboard)
...
| 2. Auth Migration | 3/3 | Complete | 2026-03-06 |
| 3. Data Migration | 2/2 | Complete | 2026-03-06 |
...
| 7. Payload Removal + Cutover | 5/5 | Complete | 2026-03-12 |
